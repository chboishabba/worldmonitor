...
async function fetchText(url, { accept = 'text/plain, text/html, application/json', timeoutMs = 30_000 } = {}) {
  try {
    ...
    const explicitHttp = proxyAuth.startsWith('http://');
    ...
    await new Promise((resolve, reject) => {
      ...
      const onData = (chunk) => {
        ...
        if (!statusLine.startsWith('HTTP/1.1 200') && !statusLine.startsWith('HTTP/1.0 200')) {
          proxySock.destroy();
          return reject(new Error(`Proxy CONNECT: ${statusLine}`));
        }
        proxySock.pause();
        resolve();
      };
    ...
    return new Promise((resolve, reject) => {
      ...
      https.request({
        ...
        if (resp.statusCode < 200 || resp.statusCode >= 300) {
          resp.resume();
          return reject(new Error(`HTTP ${resp.statusCode} via proxy`));
        }
        const chunks = [];
        ...
        resp.on('end', () => {
          ...
          (isGzip ? gunzip(body) : Promise.resolve(body))
            .then((data) => resolve({ text: data.toString('utf8'), contentType: resp.headers['content-type'] || '' }))
            .catch(fail);
        });
        resp.on('error', fail);
      }).on('error', fail).end();
